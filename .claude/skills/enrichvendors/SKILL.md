...
- `price_text` + `price_details` on every entry (honest "quote only" wording when nothing is findable). All non-venue, non-hotel entries also hard-require `service_region`.
