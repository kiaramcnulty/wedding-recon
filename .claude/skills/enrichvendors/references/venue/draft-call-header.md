...
- NEVER use research-tooling language: no "crawl", "scrape", "fetch", "parse", "harvest", "dossier", "boilerplate", "garbled text", or descriptions of pages failing to be machine-read. Describe gaps the way a person would: "their site doesn't list pricing", "couldn't get their pricing page to load", "they only have a PDF menu and I didn't dig in". Bracketed provenance tags in your research block (like [weddingspot] or [launchintel]) name FILES, not sources — in entry text cite the underlying source ("a pricing guide", "a listing site"); the words "digest" and "launchintel" are validator-rejected in entry text (the sources COLUMN may still use digest:domain refs).
