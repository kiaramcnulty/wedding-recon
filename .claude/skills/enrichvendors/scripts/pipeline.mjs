...
// All commands accept --type <venue|photographer|caterer|music|flowers|dress|planner|hairmakeup> (default venue).
//
...
if (!workdir || workdir.startsWith('--') || !CMDS.includes(cmd)) {
  console.error(`usage: pipeline.mjs <workdir> <${CMDS.join('|')}> [--type venue|photographer|caterer|music|flowers|dress|planner|hairmakeup] [flags]`); process.exit(1);
}
const profile = etype();
