...
for (const v of targets) {
  ...
  if (alreadyHarvested(v, dir)) { skipped++; continue; }
...
console.log(`\nharvested ${ok + failed} venues${skipped ? ` | ${skipped} already on file (--refresh to re-fetch)` : ''} → ${path.join(workdir, 'research')}/<slug>/harvest.json`);
