...
alter table recon_entries
  add constraint recon_collected_month_check check (recon_collected_month is null or (recon_collected_month >= 1 and recon_collected_month <= 12))
  add constraint recon_collected_year_check check (recon_collected_year is null or recon_collected_year >= 2000);
