...
**Batch-API drafting (built 2026-07-13):** `/enrichvendors` drafting can run on the metered Anthropic Batch API (off the Claude subscription, ~$1–1.50 per 300-vendor run): `pipeline.mjs batch --mode api` + `.claude/skills/enrichvendors/scripts/draft.mjs` (submit / status / collect — cost-gated, resumable, targeted resubmits). Needs `ANTHROPIC_BATCH_API_KEY` in `.env.local` (deliberately **not** `ANTHROPIC_API_KEY`, which would shadow Claude Code's own auth). Design + as-built notes in `docs/anthropic-batch-drafting.md`; pending Kiara's local 2-vendor toy test before the first full-size run. Harness draft-workers remain the no-key fallback.
