...
**Known outstanding:**
- **The LIVE Stripe webhook endpoint is not correctly registered** (found 2026-09-20), so no paid vendor has ever been mirrored into `vendor_subscriptions` and `verified_vendor_ids()` returns nobody. The code side is fixed and self-healing; the remaining work is two **dashboard** steps only Kiara can do -- register `https://www.weddingrecon.com/api/stripe-webhook` (the `www` host) in **live** mode against all six `HANDLED_EVENT_TYPES`, then resend the failed deliveries. Verify with `npm run check:stripe-webhook` using a live key. Full account in `docs/notes/admin-and-portal.md`.
- **98 of 2,301 vendors have no `location`** (all `source=user`), so they are invisible on the Explore map and in vendor search. The hole that created them is fixed (`lib/vendor/manual-entry.ts`, guarded in both forms AND both server actions), but the existing rows are **not backfilled** -- they need a geocode or a city centroid. One of them was a paying verified vendor. See `docs/notes/admin-and-portal.md`.
