...
| Admin flag, the Vendor Verification paid tier / portal, the public `/for-vendors` entry point | `docs/notes/admin-and-portal.md` |
...
- `app/for-vendors/` — the **public vendor-side marketing page** (statically prerendered, at the root like `/terms` so it does not mount `<MarkVisited>`). It is the pitch a vendor reads before signing up; `app/(portal)/` behind it is the signed-in dashboard.
