...
- **Vendor categories**: import from `lib/constants/categories.ts` (`VENDOR_TYPES`, `CATEGORIES`, `RECON_TYPES`, labels). Each type has a fixed icon + color reused on pins, chips, accordions, tags. Never hardcode category colors elsewhere. **Music was split into two types — `dj` ("DJs") and `band` ("Live music", the broad live-performer bucket: bands, quartets, soloists, pianists, ceremony ensembles). `music` is now a hidden legacy value (`LEGACY_VENDOR_TYPES`), kept in the enum + `CATEGORIES` only so a straggler row still renders.** **`beauty` ("Hair & makeup", Sparkles icon, teal — migration `0022`) is ONE joint type for wedding-day hair AND makeup: most artists do both, and a hair-only or makeup-only artist goes in the same bucket rather than a second category.** Use `VENDOR_TYPES` for anything user-selectable (picker, Explore filter, Hub); `ALL_VENDOR_TYPES` to validate a value that might be legacy.
