...
**Batch-API drafting (built 2026-07-13; DEFAULT since 2026-07-18):** `/enrichvendors` drafting runs on the metered Anthropic Batch API by default (off the Claude subscription, ~$1–1.50 per 300-vendor run): `pipeline.mjs batch` (API mode is default; `--mode harness` opts out) + `.claude/skills/enrichvendors/scripts/draft.mjs` (submit / status / collect — cost-gated, resumable, targeted resubmits). Needs `ANTHROPIC_BATCH_API_KEY` in `.env.local` (deliberately **not** `ANTHROPIC_API_KEY`, which would shadow Claude Code's own auth); if absent, `draft.mjs submit` fails fast — fall back to `batch --mode harness` + draft-worker agents. Design + as-built notes in `docs/anthropic-batch-drafting.md`. **Raise `--max-tokens` above the 16000 default for rich multi-entry batches** — 25 florists/call truncated at 16k (`collect` refuses truncated results and prints the exact `submit --calls` resubmit), 32000 cleared most, and the richest-first FIRST call needed 64000; the metered API bills actual output only, so a generous cap is free insurance (CO flowers + caterers both hit this, 2026-07). Web-session runs: harvest is local-only (sandbox can't crawl arbitrary vendor sites — see SKILL.md "Cloud/web runs"); a committed SessionStart hook (`.claude/hooks/session-start.sh`) builds `.env.local` from environment secrets. Harness draft-workers remain the no-key fallback.
