...
All commands run from the repo root. Scripts live in `.agents/skills/enrichvendors/scripts/` and need `.env.local` (`GOOGLE_PLACES_API_KEY`, `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`); they fail fast with a clear message — relay it and stop. If Places/Supabase errors persist after one retry, stop and report.
...
```
node --env-file=.env.local .agents/skills/enrichvendors/scripts/harvest.mjs <workdir> --region <ST> --type <type> --venues "Name 1;Name 2;..."
node .agents/skills/enrichvendors/scripts/dossier.mjs <workdir> --type <type>
```
...
Cloud sessions (Codex on the web) can run **everything except harvest's vendor-site
...
- **Environment prereqs (one-time, web UI):** secrets `NEXT_PUBLIC_SUPABASE_URL`,
  ...
  SessionStart hook (`.codex/hooks/session-start.sh`) materializes `.env.local` from
...
```
node --env-file=.env.local .agents/skills/enrichvendors/scripts/pipeline.mjs <workdir> batch \
...
```
node --env-file=.env.local .agents/skills/enrichvendors/scripts/draft.mjs <workdir> submit  --batch <id>          # cost-gated (--max-cost 12 default); --max-tokens defaults to 96000 so entry-dense call files don't truncate; --dry-run to preview
node --env-file=.env.local .agents/skills/enrichvendors/scripts/draft.mjs <workdir> status  --batch <id> --wait   # polls to ended (usually <1h)
node --env-file=.env.local .agents/skills/enrichvendors/scripts/draft.mjs <workdir> collect --batch <id>          # writes worker JSONLs + <id>-flags.json, prints actual $ cost
```
...
```
node --env-file=.env.local .agents/skills/enrichvendors/scripts/pipeline.mjs <workdir> status --type <type> --batch <id>
node --env-file=.env.local .agents/skills/enrichvendors/scripts/pipeline.mjs <workdir> merge  --type <type> --batch <id>
node --env-file=.env.local .agents/skills/enrichvendors/scripts/upload.mjs <workdir> --type <type> --roster <roster> --csv recons-<id>.csv   # dry-run validation
```
...
```
node --env-file=.env.local .agents/skills/enrichvendors/scripts/bots.mjs <workdir> --roster <roster>            # dry-run
node --env-file=.env.local .agents/skills/enrichvendors/scripts/bots.mjs <workdir> --roster <roster> --apply    # after approval
```
...
```
node --env-file=.env.local .agents/skills/enrichvendors/scripts/upload.mjs <workdir> --type <type> --roster <roster> --csv recons-<id>.csv --apply
node --env-file=.env.local .agents/skills/enrichvendors/scripts/pipeline.mjs <workdir> verify --type <type> --roster <roster> --csv recons-<id>.csv [--fix-gaps]
```
