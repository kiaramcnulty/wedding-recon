...
//
// usage: node --env-file=.env.local .agents/skills/enrichvendors/scripts/draft.mjs <workdir> <cmd> --batch <id> [flags]
...
// would shadow Codex's subscription auth if it leaked into a shell environment.
...
const client = () => {
  ...
  if (!key) {
    ...
    console.error('(Deliberately not ANTHROPIC_API_KEY: that name would shadow Codex\'s own auth.)');
    process.exit(1);
  }
