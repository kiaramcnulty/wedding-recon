...
## Phase 1 — sweep
**ONE query per anchor: `wedding hair and makeup near {anchor}`** (Kiara, 2026-07-26 — this is the only Places query for this type; do NOT fan out across `makeup artist` / `hair stylist` / `bridal beauty` variants, which mostly return everyday salons). Statewide launches add `wedding hair and makeup in {StateName}`. Both encoded in `TYPE_PROFILES.beauty`. Hair-only and makeup-only artists are still in scope — they come in through Phase 2 research and pastes, not through extra sweep nets.
...
```
node --env-file=.env.local .agents/skills/launchvendors/scripts/wedcheck.mjs data/launchvendors/<slug> --type hairmakeup
```
