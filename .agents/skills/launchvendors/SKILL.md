...
All commands run from the repo root. Scripts live in `.agents/skills/launchvendors/scripts/` and need `.env.local` (`GOOGLE_PLACES_API_KEY`, `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`) — they fail fast with a clear message if a key is missing; relay that to the user and stop.
...
```
node --env-file=.env.local .agents/skills/launchvendors/scripts/scout.mjs data/launchvendors/<slug> --type <type> --region "Denver, CO" [--statewide Colorado] --anchors "Boulder, CO;Golden, CO;..."
node --env-file=.env.local .agents/skills/launchvendors/scripts/ingest.mjs data/launchvendors/<slug> <scrape.csv> --type <type>   # only if the user volunteered a scrape file
```
...
```
node --env-file=.env.local .agents/skills/launchvendors/scripts/resolve.mjs data/launchvendors/<slug> --state CO --region "Denver, CO" --type <type>
```

**Resolve is a Phase-3 step and belongs BEFORE Phase 4. `adjudicate` is terminal — never run `resolve` again after it.** Resolve replays all of `candidates.jsonl`, so a re-run after adjudicating used to re-add every row you'd just removed (2026-07-29, CO beauty: 5 removed vendors resurrected + 4 renamed rows duplicated). The script now **skips any candidate already in `pruned.csv`** (by name key and by resolved `place_id`) and prints them under `N skipped as previously-pruned`, so a rejection sticks; `adjudicate --rescue` remains the only way back out. Seeing a non-zero skip count means you're re-running resolve out of order — finish resolving, then adjudicate once.
...
```
node --env-file=.env.local .agents/skills/launchvendors/scripts/adjudicate.mjs data/launchvendors/<slug> --type <type> --region "Denver, CO"
node --env-file=.env.local .agents/skills/launchvendors/scripts/adjudicate.mjs data/launchvendors/<slug> --type <type> --region "Denver, CO" \
...
```
node --env-file=.env.local .agents/skills/launchvendors/scripts/upload.mjs data/launchvendors/<slug> --type <type>            # dry-run
node --env-file=.env.local .agents/skills/launchvendors/scripts/upload.mjs data/launchvendors/<slug> --type <type> --apply    # then this, same turn
```
...
**Two dry-run lines to actually read, not skim:**
- `WARNING — instagram handle collisions` (instagram-capturing types only). The research pass hunts handles by name-searching the web, so it can staple a *different* business's handle onto a similarly-named vendor — 2026-07-29 gave "Alchemy Face Bar" the handle `@beautybarinc`, which belonged to another vendor in the same batch. A handle is an identity: if two vendors claim one, at least one is wrong. Clear the bad side before `--apply`.
- A non-zero `TO INSERT` on a workdir you already uploaded. That usually means rows were deleted at *enrich* stage (wrong-type `NOT*!` removals) while the launch CSV still lists them, so re-applying would resurrect them. `remove-vendors.mjs` now syncs those rows into the launch workdir's `pruned.csv` automatically; an older workdir may need it done by hand.
...
```
node --env-file=.env.local .agents/skills/launchvendors/scripts/backfill-websites.mjs [--type <type>] [--region CO] [--limit N]           # dry-run
node --env-file=.env.local .agents/skills/launchvendors/scripts/backfill-websites.mjs --apply [--type <type>] [--region CO] [--limit N]   # write
```
...
- **Deleted vendors stay deleted: `denylist.json`** (committed, next to `centroids.json`) lists google_place_ids deliberately removed from the hosted DB, keyed by place_id with `names`, `vendor_type`, `removed` date and a short `reason` — **business-level facts only, the repo is public**. `scout` and `resolve` skip them and `upload` hard-skips them (by place_id; a place_id-less row by normalized name within the type). It has no `--rescue` path — when you delete a vendor from the DB, add its entry in the same change. Separately, a place Google marks `CLOSED_PERMANENTLY` is moved to `pruned.csv` as `PRUNED:closed-permanently` at sweep/resolve time (rescuable, since Google can be wrong).
