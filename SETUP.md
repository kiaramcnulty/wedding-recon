...
6. **Test** → trigger a sign-in email (login page or guest Add-Recon flow) and confirm it arrives in the inbox, not spam, and that it carries **both** the code and the link. Test the code path from the installed PWA specifically — it is the one the link cannot serve.
